...
# ==========================================
# 🔐 الجزء الثاني: لوحة تحكم الأستاذ - الهندسة المعمقة وتشكيل الثنائيات والتوجيه الإداري
# ==========================================
    ...
    if password == "CMC_Nador_2026":
        st.success("🔓 تم تأكيد الهوية بنجاح. خوارزميات التشخيص والذكاء التوزيعي نشطة.")
        ...
        if not df.empty:
            ...
            if not df_filtered.empty:
                # حساب الإحصائيات العامة للفوج لتوظيفها في التحليل الجماعي
                ...
                # ----------------------------------------------------
                # أ: التحليل النفسي الفردي المعمق (ملخص نقاط القوة، الضعف والنزاعات)
                # ----------------------------------------------------
                st.markdown("### 👤 أولاً: التشخيص النفسي الفردي والتحليل المعمق للمتدربين")
                st.write("يقوم النظام بتحليل عميق لكل متدرب لتحديد ملامحه بدقة والوقاية من النزاعات:")
                ...
                for idx, row in df_filtered.iterrows():
                    with st.expander(f"👤 المتدرب(ة): {row['Nom Complet']} (الفوج الفرعي: {row['Groupe']})"):
                        ...
                        forces_list = []
                        faiblesses_list = []
                        conflits_text = "💚 بروفايل متزن ومستقر سيكولوجياً"
                        
                        if row['Conscientiousness'] >= 3.5:
                            forces_list.append("دقة متناهية وحس تنظيم محاسبي صارم")
                            faiblesses_list.append("البطء المحتمل بسبب المثالية المفرطة")
                        else:
                            forces_list.append("مرونة عالية وسرعة التكيف مع المعطيات الحسابية المتغيرة")
                            faiblesses_list.append("عرضة لارتكاب أخطاء السهو وعرقلة التخطيط مسبقاً")
                            
                        if row['Extraversion'] >= 3.5:
                            forces_list.append("مهارات تواصلية متميزة والقدرة على القيادة")
                            faiblesses_list.append("التسرع في اتخاذ القرارات دون مراجعة عميقة")
                        else:
                            forces_list.append("تركيز تحليلي صامت وعميق وحل المسائل في هدوء")
                            faiblesses_list.append("صعوبة التعبير عن التعثرات والميل للانعزال عن المجموعة")
