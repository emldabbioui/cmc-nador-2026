...
if page == "📝 Espace Stagiaire (التقويم التشخيصي)":
    ...
    with st.form("diagnostique_form"):
        ...
        q2 = st.slider("2. Je préfère travailler seul sur un projet complexe et je me sens vite fatigué par les discussions collectives.", 1, 5, 3)
        ...
        if submitted:
            if not nom or not groupe:
                ...
                # Injection dans la mémoire de l'application
...
# ==========================================
# 🔐 Partie II : Tableau de Bord Formateur et Détection des Risques
# ==========================================
            ...
            if df_filtered.empty:
                ...
                for idx, row in df_filtered.iterrows():
                    if row['Agreeableness'] <= 2.5 and row['Extraversion'] >= 3.5:
                        alerts_found = True
                        st.error(f"🔴 **Risque de conflit direct : {row['Nom Complet']} ({row['Groupe']})**\n\nForte extraversion + faible amabilité. Ce stagiaire peut chercher à imposer ses idées de manière agressive lors des travaux d'équipe (ex: Simulations, Audit) et créer des tensions.")
                    if row['Openness'] >= 4.0 and row['Conscientiousness'] <= 2.2:
                        alerts_found = True
                        st.warning(f"🔶 **Risque d'indiscipline : {row['Nom Complet']} ({row['Groupe']})**\n\nEsprit très libre mais très désorganisé. Risque de refuser de suivre les règles rigides du Plan Comptable Marocain ou des téléprocédures.")
